...
def date_from_file_name(file_name, return_datetime=False):
    """Returns full date as a string from a date in a filename.

    Args:
        file_name (str): Formatted as: Trade E-mail Archive - yyyy.mm.dd.xlsm
        return_type (bool, optional): TRUE if return should be datetime
...
def copy_worksheet(ws_input,ws_output):
    """Copies values from one openpyxl Worksheet to another.
